...
1. Если Access Token не истек, то:
   1. Клиент успешно прошел авторизацию.
2. Иначе, если Refresh Token не истек, то:
   1. Сервер обновляет Access Token и Refresh Token и высылает клиенту;
3. Иначе:
   1. Клиент должен пройти этап Аутентификации повторно;
